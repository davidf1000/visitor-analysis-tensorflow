...
min_conf_threshold = 0.525
imW, imH = 400, 300 #resolution 
use_TPU = False
...
while True:
    ...
    for i in range(len(scores)):
        ...
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)) and condglobal :#and ((labels[int(classes[i])] == 'motorcycle')):
            if (condperson):
            ...
            # Person -> person [0]
            # bicicle and motor cycle -> roda dua
            #print("classes"+str(int(classes[i])))
    ...
    cv2.putText(frame,'countperson: {} ,countcar= {}'.format(int(countperson),int(countcar)),(10,15),cv2.FONT_HERSHEY_SIMPLEX,0.6,(255,255,0),2,cv2.LINE_AA)
